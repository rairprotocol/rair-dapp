...
const insertTokens = async (tokens, contract, dbModels) => {
	...
	for await (let token of tokens) {
		...
		if (metadata && metadata.image && token.owner_of) {
			...
			let newToken = await (new dbModels.MintedToken({
	...
	importContractData: async (networkId, contractAddress, userData, dbModels) => {
		try {
			...
			if (!contract) {
				...
				let timesNeeded = Math.round(allNFTs.total / allNFTs.page_size);
				...
				let product = await (new dbModels.Product({
				...
				let offer = await new dbModels.Offer({
					offerIndex: 0,
					contract: contract._id,
					product: 0,
					offerPool: 0,
				...
				let offerPool = await (new dbModels.OfferPool({
