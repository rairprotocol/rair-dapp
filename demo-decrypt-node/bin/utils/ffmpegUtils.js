...
const convertToHLS = async (mediaData, socketInstance) => {
	...
	const promise = new Promise(async (resolve, reject) => {
		try {
			...
			let videoConversion = await spawn(ffmpeg.path, ['-i', `${mediaData.path}`].concat(...resolutionConfigs));
...
const generateThumbnails = async (mediaData, socketInstance) => {
	...
	if (mediaData.type === 'video') {
		try {
			let staticThumbnail = await spawnSync(ffmpeg.path, ['-i', `${mediaData.path}`].concat(generalThumbnailData, thumbnailParams));
			socketInstance.emit('uploadProgress', {
				message: `${ mediaData.originalname } generating static thumbnail`,
				last: false,
				done: 10
			});
			let animatedThumbnail = await spawnSync(ffmpeg.path, ['-i', `${mediaData.path}`].concat(generalThumbnailData, animatedThumbnailParams));
			socketInstance.emit('uploadProgress', {
