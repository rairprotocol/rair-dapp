...
(async function () {
  ...
  await db.collection('Offer').createIndex({ offerPool: 1, rangeIndex: 1 }, { background: true, unique: true });
  await db.collection('Offer').createIndex({ contract: 1, offerPool: 1, rangeIndex: 1 }, {
    background: true,
    unique: true
  });
