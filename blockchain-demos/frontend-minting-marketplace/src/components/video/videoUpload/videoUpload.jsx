...
const FileUpload = ({ address, primaryColor, textColor }) => {
  ...
  const getProduct = async () => {
    const responseProduct = await (
      await fetch(`api/contracts/${contract}/products`, {
        method: "GET",
        headers: {
          Accept: "application/json",
          "X-rair-token": currentToken,
    ...
    const names = responseProduct.products?.map((product) => ({
      value: product.name,
      label: product.name,
    }));
    setProductOptions(names);

    const collectionIndexInContract =
      responseProduct.products[0]?.collectionIndexInContract;
      ...
      >
        ...
        <div className="col-8 py-1">
          <InputSelect
            ...
            getter={contract}
            setter={setContract}
            placeholder="Choice your product"
