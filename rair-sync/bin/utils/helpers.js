...
const getABIData = (abi, type, eventName) => {
  const [resultingAbi] = abi.filter((item) => item.type === type && item.name === eventName);
  const instance = new ethers.Contract(ethers.constants.AddressZero, abi);
  const [topic] = Object.keys(instance.filters).filter((item) => item.includes(`${eventName}(`)).map((item) => ethers.utils.id(item));
