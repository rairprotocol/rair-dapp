...
const contractAddresses: ContractAddressesType = {
	'0x38': { // Binance Mainnet
		...
		diamondFactory: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0x556a3Db6d800AAA56f8B09E476793c5100705Db5') as string,
		diamondMarketplace: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0x92FBe344513e108B581170E73CFA352B729E47EA') as string,
	},
	'0x61': { // Binance Testnet
		...
		diamondFactory: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0xA2c57691b8DF0D8479f5f888c69346363D23a49F') as string,
		diamondMarketplace: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0xaCb13B4c527eD6237f7DB6E95Ef71929d1e13DD6') as string,
	},
	'0x5': { // Ethereum Goerli
		...
		diamondFactory: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0xEF85370b8F136E2F28eA904bF0dA5acac3D1d74f') as string,
		diamondMarketplace: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0x6B3c06b39Aa1ADe73c625b184326d4837c7a2b64') as string,
	},
	'0x13881': { // Matic Mumbai
		...
		diamondFactory: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0xbB236Ce48dDCb58adB8665E220FE976bA5d080a5') as string,
		diamondMarketplace: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0x2c8BA9f098CD319a971cE2705F26723c81044Cb0') as string,
	},
	'0x89': { // Matic Mainnet
		factory: '0x701931758cB94F9AA684e13f710F5e4B85Bb94F2',
		erc777: '0x0Ce668D271b8016a785Bf146e58739F432300B12',
		minterMarketplace: '0x781F15a23506CF28539EA057e3f33008E6339E49',
		diamondFactory: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0x9498b23e964760364435C23c793e9352Ff4E2200') as string,
		diamondMarketplace: (process.env.REACT_APP_DIAMONDS_ENABLED === 'true' && '0x51eA5316F2A9062e1cAB3c498cCA2924A7AB03b1') as string,
	},
...
export default function userStore(state: ContractsInitialType = InitialState, action: ContractsActionsType): ContractsInitialType {
	switch (action.type) {
		case types.SET_CHAIN_ID:
			if (action.currentChain !== undefined && contractAddresses[action.currentChain] !== undefined) {
				let signer;
				if (window.ethereum) {
				...
				return {
					...state,
					currentChain: action.currentChain,
					factoryInstance: contractCreator(contractAddresses[action.currentChain].factory, factoryAbi),
					minterInstance: contractCreator(contractAddresses[action.currentChain].minterMarketplace, minterAbi),
					erc777Instance: contractCreator(contractAddresses[action.currentChain].erc777, erc777Abi),
					diamondFactoryInstance: contractCreator(contractAddresses[action.currentChain].diamondFactory, diamondFactoryAbi),
					diamondMarketplaceInstance: contractCreator(contractAddresses[action.currentChain].diamondMarketplace, diamondMarketplaceAbi),
