...
export function* setNftDataContract({ params, type }: TParamsNftDataProps) {
    ...
    try {
        const { data }: AxiosResponse<TGetFullContracts> = yield call(axios.get, `/api/contracts/full?itemsPerPage=${params.itemsPerPage}&pageNum=${params.currentPage}` + `${params.blockchain ? "&blockchain=" + params.blockchain : ""}` + `${params.category ? "&category=" + params.category : ""}`);
