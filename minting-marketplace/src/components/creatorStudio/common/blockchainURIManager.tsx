...
const TokenURIRow = ({tokenId, metadataURI, deleter, index, array, lastTokenInProduct}) => {
...
const BlockchainURIManager = ({contractData, address, collectionIndex}) => {
	...
	const [blockchainOperationInProgress, setBlockchainOperationInProgress] = useState(false);
	...
	const [appendTokenForCollection, setAppendTokenForCollection] = useState(false);
	...
	useEffect(() => {
		if (contractData?.product?.offers) {
			setRangeOptions(contractData.product.offers.map(item => {
				return {
	...
	const addUniqueURIRow = () => {
		...
		let aux = [...uniqueURIArray];
		let nextToken = contractData.nfts.tokens[uniqueURIArray.length];
		if (!nextToken) {
			nextToken = {
				uniqueIndexInContract: uniqueURIArray.length > 0 ? uniqueURIArray.at(-1).tokenId + 1 : 0,
				metadataURI: ""
			}
		}
		aux.push({
	...
	return <>
		...
		<details>
			...
			<div className='w-100 row'>
				Set the same metadata URI for any token with no unique, range or collection URI {appendTokenForContract && <>(The token's index will be appended at the end of the URI)</>}
				...
				<button
					...
					className={`col-12 col-md-6 btn btn-${appendTokenForContract ? 'royal-ice' : 'warning'}`}>
				...
				<button
					onClick={async e => {
						...
						if (await metamaskCall(
							contractData.instance.setBaseURI(contractWideMetadata, appendTokenForContract)
						)) {
					...
					{blockchainOperationInProgress ? 'Sending Metadata...' : `${contractWideMetadata === '' ? 'Uns' : 'S'}et metadata`}
				</button>
			</div>
		</details>
		<hr />
		<details>
			...
			<div className='w-100 row'>
				Set the same metadata URI for any token with no unique or range URI {appendTokenForCollection && <>(The token's index will be appended at the end of the URI)</>}
				<div className='col-12'>
					<InputField
						customClass='form-control'
				...
				<button
					onClick={() => setAppendTokenForCollection(!appendTokenForCollection)}
					className={`col-12 col-md-6 btn btn-${appendTokenForCollection ? 'royal-ice' : 'warning'}`}>
				...
				<button
					onClick={async e => {
						...
						setBlockchainOperationInProgress(true);
						console.log(contractData.instance.functions)
						if (await metamaskCall(
							contractData.instance.setProductURI(collectionIndex, collectionWideMetadata, appendTokenForCollection)
						)) {
					...
					disabled={blockchainOperationInProgress || !contractData.instance}
					className='col-12 col-md-6 btn-stimorol btn'>
					{blockchainOperationInProgress ? 'Sending Metadata...' : `${collectionWideMetadata === '' ? 'Uns' : 'S'}et metadata`}
				</button>
			</div>
		</details>
		<hr />
		<details>
			...
			<div className='w-100 row'>
				Set the same metadata URI for any token with no unique URI {appendTokenForRange && <>(The token's index will be appended at the end of the URI)</>}
				<div className='col-12'>
					<InputSelect
						customClass='form-control'
				...
				<button
					...
					className={`col-12 col-md-6 btn btn-${appendTokenForRange ? 'royal-ice' : 'warning'}`}>
				...
				<button
					onClick={async e => {
						...
						if (await metamaskCall(
							contractData.instance.setRangeURI(selectedRange, rangeWideMetadata, appentTokenForRange),
					...
					disabled={blockchainOperationInProgress || !contractData.instance || selectedRange === "null"}
					className='col-12 col-md-6 btn-stimorol btn'>
					{blockchainOperationInProgress ? 'Sending Metadata...' : `${rangeWideMetadata === '' ? 'Uns' : 'S'}et metadata`}
				</button>
			</div>
		</details>
		<hr />
		<details>
			...
			<div className='col-12'>
				...
				<table style={{color: 'inherit'}} className='table table-responsive w-100'>
					<thead>
						<tr>
							...
							<th className='px-0'>
								<button onClick={addUniqueURIRow} className='btn btn-stimorol'>
				...
				<button
					onClick={async e => {
						...
						if (await metamaskCall(
							contractData.instance.setUniqueURIBatch(
								uniqueURIArray.map(item => BigNumber.from(contractData.product.firstTokenIndex).add(item.tokenId)),
					...
					disabled={blockchainOperationInProgress || uniqueURIArray.length <= 0} className='col-12 btn btn-primary'>
					{blockchainOperationInProgress ? 'Please wait...' : `Update the URI of ${uniqueURIArray.length} tokens`}
				</button>
			</div>
		</details>
		<hr />
			...
			<div className='w-100 row'>
				...
				<div className='col-12 col-md-3'>
					<button
						...
						disabled={!contractData.instance} className='btn-stimorol w-100 btn'>
						{blockchainOperationInProgress ? 'Sending Metadata...' : `${openSeaURI === '' ? 'Uns' : 'S'}et contract metadata`}
					</button>
				</div>
			</div>
		</details>
