...
const FactoryManager: React.FC<IFactoryManager> = ({ setDeployedTokens }) => {
	...
	const {erc777Instance, factoryInstance, currentUserAddress} = useSelector<RootState, ContractsInitialType>(state => state.contractStore);
	...
	return <div className='col py-4 border border-white rounded' style={{position: 'relative'}}>
		...
		<br />
		{(tokensOwned && tokensRequired && tokenDecimals) ? <>You currently own {tokensOwned._hex.length} ERC721 contracts<br/>
