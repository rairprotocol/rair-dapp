...
const VideoList: React.FC<IVideoList> = ({ titleSearch, primaryColor }) => {
	const { videos, loading } = useSelector<RootState, TVideosInitialState>(store => store.videosStore);
		...
		<div className='list-button-wrapper' style={{ verticalAlign: 'top' }}>
			{videos ?
				Object.keys(videos).length > 0 ?
					Object.keys(videos).filter(item => videos[item].title.toLowerCase().includes(titleSearch.toLowerCase())).map((item, index) => {
