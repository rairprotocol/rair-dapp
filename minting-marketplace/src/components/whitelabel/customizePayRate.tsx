...
const ModalContent = ({ instance, catalogIndex }) => {
  ...
  return (
    <div className="row px-0 mx-0">
      ...
      <div
        className="col-12 row px-0 mx-0 my-3"
      ...
      <button
        ...
        onClick={async () => {
          ...
          try {
            await (
              await instance.setCustomPayment(
                catalogIndex,
                customPayments.map((i) => i.receiver),
                customPayments.map(
                  (i) => i.percentage * Math.pow(10, minterDecimals)
                )
              )
            ).wait();
          } catch (err) {
...
const CustomizePayRate = ({
  ...
  const { minterInstance, programmaticProvider, contractCreator } = useSelector(
