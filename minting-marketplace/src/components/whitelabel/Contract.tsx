...
const Contract = ({ contractAddress, blockchain /*, title*/ }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    const response2 = await rFetch(
      `/api/contracts/network/${blockchain}/${contractAddress}`
    );
    const response3 = await rFetch(
      `/api/contracts/network/${blockchain}/${contractAddress}/products`
    );
    if (response3.success) {
      response2.contract.products = response3.products;
    }
    const response4 = await rFetch(
      `/api/contracts/network/${blockchain}/${contractAddress}/products/offers`
    );
      ...
      <div
        ...
        {!data ? (
          ...
          <>
            ...
            <h5>{data.title}</h5>
            <details>
              <summary>{data.products?.length} products!</summary>
              <hr />
              <CreateProduct
                  ...
                  .map((product, index) => {
                    return (
                      <div
                        ...
                        <Link
                          to={`/rair/${blockchain}/${data.contractAddress}/${product.collectionIndexInContract}`}>
                          {product.name}
                        </Link>
                        <br />
                        ...
                        <details className="w-100 text-start row px-0 mx-0 pt-0 mb-2">
                          <summary>
                            {product.offers ? product.offers.length : 'No'}{' '}
                            offers
                          </summary>
                          {product.offers
                            ?.sort((a, b) =>
                                ...
                                <div
                                  ...
                                  <div className="row px-0 mx-0">
                                    <div className="col-12">
                                      {offer.soldCopies} of{' '}
                                      {Number(offer.range[1]) -
                                        Number(offer.range[0]) +
                                        1}{' '}
                                      tokens sold
                                    </div>
                                    <LockRange
