...
const { getTransactionHistory, getLatestBlock } = require('../utils/logUtils');
    ...
    async (task, done) => {
      ...
      // Log the start of the task
        ...
        if (version.running === true) {
          return done({ reason: `A ${AgendaTaskEnum.SyncDiamondContracts} process for network ${network} is already running!` });
        }
        ...
        if (latestBlock < version.number) {
          log.error(`ERROR: ${AgendaTaskEnum.SyncDiamondContracts} latest block is ${version.number} but the last mined block from ${network} is ${latestBlock}, the contract's last synced block will be reset to 0, expect a lot of duplicate transaction messages!`);
          version.number = 0;
        }
        ...
        const processedResult = await getTransactionHistory(
          networkData.diamondFactoryAddress,
          network,
          version.number,
        );
        ...
        for await (const [event] of processedResult) {
          ...
          if (
            ...
            filteredTransaction.toAddress.includes(networkData.diamondFactoryAddress)
          ) {
          ...
          // If the log is already on DB, update the address list
            ...
            try {
              const documentToInsert = await event.operation(
                ...
                // I do this because previously the deployment event
                // from the classic and the diamond contracts had the
                // same name, so if by syncing old data I find the old
                // name, it'll register it as a classic event instead
                // of a diamond, forcing it to true here resolves this conflict
