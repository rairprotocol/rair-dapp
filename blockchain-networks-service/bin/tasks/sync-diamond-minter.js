...
module.exports = (context) => {
	...
	context.agenda.define(AgendaTaskEnum.SyncDiamondMarketplaceEvents, { lockLifetime }, async (task, done) => {
		try {
			...
			let version = await context.db.Versioning.findOne({ name: AgendaTaskEnum.SyncDiamondMarketplaceEvents, network });
			...
			*/
			...
			const options = {
				address: networkData.diamondMarketplaceAddress,
				chain: network,
			...
			log.info(`[${network}] Found ${logData.total} events on diamond minter marketplace since block #${version.number}`);
