  ...
  context.agenda.define(
    ...
    async (task, done) => {
      try {
        ...
        logAgendaActionStart({ agendaDefinition: AgendaTaskEnum.SyncDiamondMarketplaceEvents });
        ...
        if (!networkData.diamondMarketplaceAddress) {
          log.info(`Skipping ${network}, diamond marketplace address is not defined.`);
          return done();
        }

        // Get last block parsed from the Versioning collection
        let version = await Versioning.findOne({
          name: AgendaTaskEnum.SyncDiamondMarketplaceEvents,
          network,
        });
        ...
        if (latestBlock < version.number) {
          log.error(`ERROR: ${AgendaTaskEnum.SyncDiamondMarketplaceEvents} latest block is ${version.number} but the last mined block from ${network} is ${latestBlock}, the contract's last synced block will be reset to 0, expect a lot of duplicate transaction messages!`);
          version.number = 0;
        }
        ...
        const processedResult = await getTransactionHistory(
          networkData.diamondMarketplaceAddress,
          network,
          version.number,
        );
        ...
        for await (const [event] of processedResult) {
          ...
          if (
            ...
            filteredTransaction.toAddress.includes(networkData.diamondMarketplaceAddress)
          ) {
        ...
        log.info(`Done with ${network}, ${AgendaTaskEnum.SyncDiamondMarketplaceEvents}`);
